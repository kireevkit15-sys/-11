...
import { partnerTags as staticTags } from '@/data/partners'
import type { Partner } from '@/data/partners'
import { cn } from '@/lib/utils'

// Данные приходят из RSC (см. server-обёртку app/announcements/page.tsx) и
// совпадают на SSR и client. useHydrated больше не нужен — нет fetch.
...
export function AnnouncementsView({ initialPartners }: { initialPartners: Partner[] }) {
  ...
  // Нет client fetch — SSR и client видят один список. Новый партнёр, добавленный
  // в admin, появляется в HTML сразу после рестарта контейнера / on-demand revalidate.
