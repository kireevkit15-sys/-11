...
// занятый другим стеком. На сервере используем абсолютный URL.
// NB: внутри контейнера web этот URL должен указывать на собственный
// origin контейнера (порт 3000), а не на хостовый маппинг (например 3300).
// Хостовый маппинг снаружи Docker Desktop: из контейнера localhost:<host-port>
// НЕ резолвится. Поэтому отдельный INTERNAL_API_BASE через env.
const API_BASE = typeof window === 'undefined'
  ? (process.env.INTERNAL_API_BASE ?? process.env.NEXT_PUBLIC_SITE_URL ?? '')
  : '';
