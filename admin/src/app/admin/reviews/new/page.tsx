...
export default function NewReviewPage() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Новый отзыв</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" value={form.authorName} onChange={e => setForm(f => ({ ...f, authorName: e.target.value }))} placeholder="ООО Стартап" className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Проект</label>
            <input type="text" value={form.authorProject} onChange={e => setForm(f => ({ ...f, authorProject: e.target.value }))} placeholder="EdTech-платформа" className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500" />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Текст отзыва *</label>
            <textarea value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))} rows={4} placeholder="Отличный сервис! Помогли открыть ООО за 3 дня." className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 resize-y" />
          </div>
          ...
          <div>
            ...
            <input type="url" value={form.sourceUrl} onChange={e => setForm(f => ({ ...f, sourceUrl: e.target.value }))} placeholder="https://vk.com/wall-123" className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500" />
          </div>
