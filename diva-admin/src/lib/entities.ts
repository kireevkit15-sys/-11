...
/**
 * Фиксированный список категорий партнёров — те же 18 значений, что и
 * `partnerTags` на сайте (web/src/data/partners.ts). Это единый список
 * для фильтров /announcements и для multiselect в форме партнёра.
 * Список захардкожен намеренно: эти строки — контракт между админкой
 * и сайтом; если их поменять, надо править оба места и существующие
 * данные в БД.
 */
...
export type FieldType =
  ...
  | 'checkbox'
  | 'select'
  | 'multiselect' // массив строк из фиксированного options (jsonb)
...
export const ENTITIES: Record<string, EntityConfig> = {
  ...
  partners: {
    ...
    fields: [
      ...
      // Множественные категории — те же строки, что и partnerTags на
      // сайте. Выбранные значения попадают в jsonb-колонку categories и
      // становятся фильтрами на /announcements.
      { name: 'categories', label: 'Категории (фильтры на сайте)', type: 'multiselect', options: [...PARTNER_CATEGORIES], help: 'Выберите одну или несколько — они станут кликабельными фильтрами в объявлениях', group: 'Основное' },
      { name: 'category', label: 'Категория (внутренняя)', type: 'select', options: ['fullstack', 'mobile', 'ai', 'devops', 'design', 'other'], help: 'Внутренняя категория для сортировки/админки (на сайте не видна)', group: 'Основное' },
