  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        // router.refresh() обновляет данные списка на фоне, но НЕ перечитывает саму
        // форму (loadRecord зависит только от [id], см. комментарий выше), поэтому
        // без явного loadRecord() форма показала бы локальный стейт, а не то, что
        // реально легло в БД. Перечитываем запись, чтобы UI отражал истину — это
        // устраняет иллюзию «редактирование не применилось» и показывает серверные
        // нормализации (trim) и актуальные значения полей.
